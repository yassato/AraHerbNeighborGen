...
# for a single file
gwas_out = read.csv("../output/BranchNoNeiGWASbolting_CHZ2019_scaledLMM.csv",header=TRUE)
gwas_out = gwas_out[gwas_out$P_nei<quantile(gwas_out$P_nei,0.001),]
gwas_out = gwas_out[gwas_out$beta_nei>0,]

gwas_SNP = paste0(gwas_out$Chr,"-",gwas_out$Pos)
ehh_SNP = paste0(ehh_out$CHR,"-",ehh_out$POSITION)
beta_SNP = paste0(beta_out$Chr,"-",beta_out$Position)

length(intersect(ehh_SNP,gwas_SNP))
length(intersect(beta_SNP,gwas_SNP))

# for all files
fn = system("ls ../output/*neiGWAS*.csv", intern=TRUE)
...
# repanel (J=4)
repanel = rbind(res[1,], res[7,], res[13,], res[10,],
                res[17,], res[19,], res[23,], res[21,])
...
h1 = ggplot(bar[1:16,],aes(y=value,x=trait,fill=group))+geom_bar(stat="identity", position="dodge") + labs(title="(C) Zurich") +
...
h2 = ggplot(bar[17:32,],aes(y=value,x=trait,fill=group))+geom_bar(stat="identity", position="dodge") + labs(title="(D) Otsu") +
...
scan_hist = (h1 | h2)
saveRDS(scan_hist,file="../figs/SelectionSNPno.rds")

ggsave(scan_hist,filename="../figs/SelectionSNPno.pdf",width=16,height=4)


# histograms
s1 = list()
fn = system("ls ../output/*_*S1.csv", intern=TRUE)
for(i in fn[c(1,3,5,4,7:8,10,9)]) {
  gwas_out = read.csv(i, header=TRUE)
  gwas_out = gwas_out[gwas_out$P_nei<quantile(gwas_out$P_nei,0.001),]
  p = ggplot(gwas_out,aes(x=beta_nei))+geom_histogram()+theme_classic()+ylab("")+xlab("")
  s1 = append(s1,list(p))
  #hist(gwas_out$beta_nei,ylab="",xlab="",main="",las=1)
}

s2 = list()
fn = system("ls ../output/*_*S2.csv", intern=TRUE)
for(i in fn[c(1,3,5,4,7:8,10,9)]) {
  gwas_out = read.csv(i, header=TRUE)
  gwas_out = gwas_out[gwas_out$P_nei<quantile(gwas_out$P_nei,0.001),]
  p = ggplot(gwas_out,aes(x=beta_nei))+geom_histogram()+theme_classic()+ylab("")+xlab("")
...
# repanel (J=12)
repanel = rbind(res[2,], res[8,], res[14,], res[11,],
                res[18,], res[20,], res[24,], res[22,])
repanel$trait = factor(repanel$trait,levels=repanel$trait)
...
h1 = ggplot(bar[1:16,],aes(y=value,x=trait,fill=group))+geom_bar(stat="identity", position="dodge") + labs(title="(E) Scan at Zurich (J = 12)") +
  theme_classic() + ylab("No. of SNPs") + xlab("") + scale_x_discrete(labels=c("Leaf holes","External feeders","Internal feeders","Species number")) +
  scale_fill_manual(values=c("red", "blue", "pink","skyblue"),name="Category",
                    breaks=c("beta2Pos_EHH", "beta2Pos_Beta", "beta2Neg_EHH", "beta2Neg_Beta"),
                    labels=c(expression(iHS:italic(beta)[2]>0), expression(BETA:italic(beta)[2]>0), expression(iHS:italic(beta)[2]<0), expression(BETA:italic(beta)[2]<0))) +
...
h2 = ggplot(bar[17:32,],aes(y=value,x=trait,fill=group))+geom_bar(stat="identity", position="dodge") + labs(title="(F) Scan at Otsu (J = 12)") +
  theme_classic() + ylab("No. of SNPs") + xlab("") + scale_x_discrete(labels=c("Leaf area loss","External feeders","Internal feeders","Species number")) +
  scale_fill_manual(values=c("red", "blue", "pink","skyblue"),name="Category",
                    breaks=c("beta2Pos_EHH", "beta2Pos_Beta", "beta2Neg_EHH", "beta2Neg_Beta"),
                    labels=c(expression(iHS:italic(beta)[2]>0), expression(BETA:italic(beta)[2]>0), expression(iHS:italic(beta)[2]<0), expression(BETA:italic(beta)[2]<0))) +
...
scan_p = (beta2_hist / scan_hist / scan_bar) + plot_layout(heights = c(2,1,1))
ggsave(scan_p, filename="../figs/AraHerbSelectionScan.pdf",width=18,height=12)
