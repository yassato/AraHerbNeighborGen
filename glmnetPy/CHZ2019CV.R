###########################################
# cross validation using 2019 Zurich data #
###########################################
...
Y = scale(log(pheno_d$Holes+1)) # change trait names when needed

# calc. correlations from beta i to all
...
plot(corlist[,2], main="no. of Holes", ylab="rho", xlab="lambda rank",ylim=c(0,0.425))
points(corlist[,3],pch=2)
points(corlist[,1],pch=20)

colnames(corlist) = c("Holes_self","Holes_bothS1","Holes_bothS2")
write.csv(corlist,"../output/HolesCHZrhoLASSO.csv",row.names=FALSE)

# summary files
fn = system("ls ../output/*CHZrhoLASSO.csv", intern=TRUE)
rholist = c()
for(i in fn) {
...
write.csv(rholist,"../output/SummaryCHZrhoLASSO.csv",row.names=FALSE)

fn = system("ls ../output/*CHZ*DFlambda.csv", intern=TRUE)
dflist = c()
for(i in fn) {
  f = read.csv(i,header=TRUE)
...
write.csv(dflist,"../output/SummaryCHZlambdaLASSO.csv",row.names=FALSE)
